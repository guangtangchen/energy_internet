...
def main_get_words_num():
    buff_all = {}
    with open('energy_internet/people_news_time_and_href.txt', 'r') as f:
        data = f.read().split('{{')
        date_and_href = []
        for item in data:
...
def get_parse_url(text):
    ...
    for item in answer:
        ...
        date = re.findall('\d\d\d\d-\d\d-\d\d', item)[0]
        date_and_href.append((href, date))
        date_all.append(date)
    print("本页面获取的条目数量：",len(date_and_href))
    with open('energy_internet/people_news_time_and_href.txt', 'a') as f:
    ...
    for date in date_all:
        if date[:4] == '2020':
            ...
            if date[:8] in buff:
                ...
                buff[date[:8]] = 1
    with open('energy_internet/people_news_num.txt', 'w') as f:
